...
2. **Profile to find where the time goes** before changing code. A sampling profiler shows the hot functions with low overhead; a tracing profiler shows exact call counts (useful to spot N+1: the same query 5,000 times).
...
- **Noise:** differences under ~5% between runs are usually noise; repeat, and use `benchstat` or hyperfine's statistics before claiming a win.
