...
```bash
python3 ~/.amazonq/complexity-optimizer/analyze_complexity.py . --format markdown   # ranked hotspots + 0-100 health score
python3 ~/.amazonq/complexity-optimizer/analyze_complexity.py . --changed main      # only files changed vs main
python3 ~/.amazonq/complexity-optimizer/measure_growth.py "python3 bench.py {n}"  # measured growth: x2 per doubling = O(n), x4 = O(n^2)
```
