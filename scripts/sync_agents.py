...
```bash
python3 {tools_dir}/analyze_complexity.py . --format markdown   # hotspots ranked by function
python3 {tools_dir}/analyze_complexity.py . --changed main      # only files changed vs main
python3 {tools_dir}/measure_growth.py "python3 bench.py {{n}}"  # measured growth exponent with a confidence interval
python3 {tools_dir}/verdicts.py add ~/.complexity-optimizer/verdicts/<repo>.jsonl "<path>::<function>::<kind>" confirmed
```
...
customCommands:
  - name: complexity-report
    ...
    prompt: |
      ...
      Run the scanner first (hotspots ranked by function):
      ```bash
