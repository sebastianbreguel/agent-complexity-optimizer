...
class TestReviewRegressions:
    QUERY_LOOP = "def g(ids, db):\n    for i in ids:\n        db.query(i)\n"

    def test_prose_about_editing_is_not_a_generated_marker(self, repo: Path):
        (repo / "order.py").write_text("# Do not edit the order of these steps by hand.\n" + self.QUERY_LOOP)
        assert "order.py" in {f["path"] for f in scan_json(repo)["findings"]}

    def test_one_long_line_does_not_hide_a_file(self, repo: Path):
        (repo / "prompt.py").write_text(f'PROMPT = "{"x" * 1500}"\n' + self.QUERY_LOOP)
        assert "prompt.py" in {f["path"] for f in scan_json(repo)["findings"]}

    def test_tool_generated_files_are_skipped_and_listed(self, repo: Path):
        (repo / "api_pb2.py").write_text("# @generated by protoc\n" + self.QUERY_LOOP)
        report = scan_json(repo)
        assert "api_pb2.py" not in {f["path"] for f in report["findings"]}
        assert report["generated_files"] == ["api_pb2.py"]

    def test_missing_root_is_an_error(self, tmp_path: Path):
        result = run_scanner(tmp_path / "typo", "--fail-on", "high", check=False)
        assert result.returncode == 2
        assert "not a directory" in result.stderr

    def test_changed_with_baseline_only_counts_scanned_files_as_fixed(self, repo: Path):
        baseline = repo / "baseline.json"
        run_scanner(repo, "--write-baseline", str(baseline))
        git(repo, "add", ".")
    ...
    def test_unparseable_python_falls_back_to_text(self, repo: Path):
        ...
        (repo / "deep.py").write_text("x = " + " + ".join(["1"] * 3000) + "\n")
        kinds = {f["path"]: f["kind"] for f in scan_json(repo)["findings"]}
        assert kinds.get("deep.py") in (None, "parse-error")
        assert "nul.py" in kinds
